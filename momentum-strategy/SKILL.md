...
- **持仓**：一周（周一开盘 → 周五收盘）
- **回测期**：2011-07-10 → 2026-02-01（约15年，13只真实股票）
...
### Regime Switch 增强（Weekly + 13只干净股票 universe，2026-05-06 新增）

**实验设计**：固定策略 vs Regime-Switching（连续 8 周 Sharpe<0 → 切换 IEF/SPLV），两者唯一区别是 Regime Switch 在防御期持有债券，其余所有逻辑完全相同。

**结果（无 TC）：**

| 指标 | 固定(A) | RegimeSw(B) | B-A |
|------|---------|-------------|-----|
| Sharpe | **1.29** | **2.11** | **+0.82** |
| 年化收益 | 24.0% | 31.9% | +7.9% |
| MaxDD | -32.5% | -16.1% | +16.4pp |
| 防御状态 | — | 169/760 (22%) | — |
| 切换次数 | — | 90 次 | — |

**TC 敏感性：**

| TC (单程) | 固定 Sharpe | RegimeSw Sharpe | B-A |
|-----------|-------------|-----------------|-----|
| 0bps | 1.29 | 2.11 | +0.82 |
| 10bps | 1.17 | 1.99 | +0.82 |
| 20bps | 1.04 | 1.87 | +0.83 |
| 30bps | 0.91 | 1.76 | +0.84 |
| 50bps | 0.67 | 1.52 | +0.85 |

> SPY B&H baseline: Sharpe=0.78, Ann=12.5%, MaxDD=-32.2%
...
5. **Regime Switch 大幅改善 Sharpe**：在 Weekly 基础上，+0.82（防御态 22% 时间躲开大跌市）；MaxDD 减半（-32.5% → -16.1%）；TC 对优势几乎无影响（B 年换手 0.17x < A 0.22x）

### ⚠️ Universe 污染陷阱（已踩坑记录）

**教训**：`rolling_retrain_weekly.py` 初始用 78 个混合资产（含 SPXL/SPXS/TVIX/VIXY 等杠杆/VIX ETF），Sharpe 只有 0.28。换成 13 只干净股票后 Sharpe = 1.29。

**原因**：杠杆 ETF（3x/-3x）和 VIX ETF 有 **path-dependency decay**（路径依赖损耗），长期持有必然亏损。混入股票池会暗中拉低任何策略的 Sharpe。

**教训**：回测时 universe 里混入非股票产品前必须三思。
